...
export async function analyticsRoutes(fastify: FastifyInstance) {
  fastify.get('/analytics/aggregates', { preHandler: verifyAnalyticsApiKey }, async (request, reply) => {
    try {
      const pool = getPool();
      ...
      return { bucket: query.bucket, items, next_cursor };
    } catch (error) {
      if (error instanceof z.ZodError) {
        return reply.code(422).send({ error: 'Validation Error', details: error.errors });
      }
      request.log.error({ err: error }, 'analytics/aggregates failed');
      return reply.code(500).send({ error: 'Internal Server Error', requestId: request.id });
    }
  });

  fastify.get('/analytics/coverage', { preHandler: verifyAnalyticsApiKey }, async (request, reply) => {
    try {
      const pool = getPool();
      ...
      request.log.error({ err: error }, 'analytics/coverage failed');
      return reply.code(500).send({ error: 'Internal Server Error', requestId: request.id });
    }
  });

  fastify.get('/analytics/device-stats', { preHandler: verifyAnalyticsApiKey }, async (request, reply) => {
    try {
      const pool = getPool();
      ...
      request.log.error({ err: error }, 'analytics/device-stats failed');
      return reply.code(500).send({ error: 'Internal Server Error', requestId: request.id });
    }
  });
}
